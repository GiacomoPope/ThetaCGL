use super::utils64::{addcarry_u64, sgnw, subborrow_u64, umull, umull_add, umull_add2};
            ...
            fn set_add_shifted(&mut self, rhs: &Self, s: u32) {
                if s < 64 {
                    if s == 0 {
                        ...
                        let (d0, mut cc) = addcarry_u64(self.0[0], rhs.0[0] << s, 0);
                        self.0[0] = d0;
                        for i in 1..Self::N {
                    ...
                    if s == 0 {
                        ...
                        self.0[j] = dj;
                        for i in (j + 1)..Self::N {
                            let (dx, ee) = addcarry_u64(self.0[i], rhs.0[i - j], cc);
                            self.0[i] = dx;
                            cc = ee;
                        }
                    } else {
                        let (dj, mut cc) = addcarry_u64(self.0[j], rhs.0[0] << s, 0);
                        self.0[j] = dj;
                        for i in (j + 1)..Self::N {
                            let bw = (rhs.0[i - j - 1] >> (64 - s)) | (rhs.0[i - j] << s);
                            let (dx, ee) = addcarry_u64(self.0[i], bw, cc);
                            self.0[i] = dx;
                            cc = ee;
                        }
            ...
            fn set_sub_shifted(&mut self, rhs: &Self, s: u32) {
                if s < 64 {
                    if s == 0 {
                        ...
                        let (d0, mut cc) = subborrow_u64(self.0[0], rhs.0[0] << s, 0);
                        self.0[0] = d0;
                        for i in 1..Self::N {
                            let bw = (rhs.0[i - 1] >> (64 - s)) | (rhs.0[i] << s);
                    ...
                    if s == 0 {
                        ...
                        self.0[j] = dj;
                        for i in (j + 1)..Self::N {
                            let (dx, ee) = subborrow_u64(self.0[i], rhs.0[i - j], cc);
                            self.0[i] = dx;
                            cc = ee;
                        }
                    } else {
                        let (dj, mut cc) = subborrow_u64(self.0[j], rhs.0[0] << s, 0);
                        self.0[j] = dj;
                        for i in (j + 1)..Self::N {
                            let bw = (rhs.0[i - j - 1] >> (64 - s)) | (rhs.0[i - j] << s);
                            let (dx, ee) = subborrow_u64(self.0[i], bw, cc);
...
macro_rules! define_lagrange {
    ($name:ident, $n0:ident, $n1:ident, $n2:ident, $n3:ident) => {
        #[allow(dead_code)]
        pub(crate) fn $name(
            k: &[u64; $n1::N],
            n: &[u64; $n1::N],
...
macro_rules! define_lagrange_spec {
    ($name:ident, $n0:ident, $n1:ident, $n3:ident) => {
        #[allow(dead_code)]
        pub(crate) fn $name(
            a0: &[u64; $n1::N],
            a1: &[u64; $n1::N],
            ...
            fn smul(a: &[u64; $n1::N], b: &[u64; $n1::N]) -> $n3 {
                ...
                for i in 0..($n3::N - $n1::N) {
                    (d.0[i + $n1::N], cc) = subborrow_u64(d.0[i + $n1::N], b[i] & sa, cc);
                }
                cc = 0;
                for i in 0..($n3::N - $n1::N) {
                    (d.0[i + $n1::N], cc) = subborrow_u64(d.0[i + $n1::N], a[i] & sb, cc);
                }
...
pub(crate) fn lagrange128_basisconv_vartime(
    a: &[u64; 2],
    b: &[u64; 2],
) -> (i64, i64, i64, i64, u32) {
