...
impl<const MQ: u64> GF255<MQ> {
    ...
    pub const TWO: GF255<MQ> = GF255::<MQ>([2, 0, 0, 0]);
    pub const THREE: GF255<MQ> = GF255::<MQ>([3, 0, 0, 0]);
    pub const FOUR: GF255<MQ> = GF255::<MQ>([4, 0, 0, 0]);
    ...
    fn set_add(&mut self, rhs: &Self) {
        ...
        let (d0, cc) = addcarry_u64(d0, (cc as u64).wrapping_neg() & (2 * MQ), 0);
    ...
    #[inline(always)]
    pub fn mul2add_mul2sub_noreduce(self, b: &Self) -> (GF255NotReduced<MQ>, GF255NotReduced<MQ>) {
        let d = self.mul2();
        let e = d + b;
        let f = d - b;
        (e, f)
    ...
    fn set_sub(&mut self, rhs: &Self) {
        ...
        let (d0, cc) = subborrow_u64(d0, (cc as u64).wrapping_neg() & (2 * MQ), 0);
    ...
    pub fn set_mul_small(&mut self, k: i32) {
        ...
        let sk = (k >> 31) as u32;
        let b = ((k as u32) ^ sk).wrapping_sub(sk) as u64;
    ...
    fn set_fourth_root(&mut self) -> u32 {
        // We can support only q = 7 mod 8
        // See compile_time_checks() for the compile-time verification
        // that MQ matches that restriction.

        // Input is denoted x in code comments.

        // IWe need to compute a modular exponentiation, and the exponent's
        //top 240 bits are all equal to one, so we have a common part to compute
        // z^(2^240-1), with z being the exponentiated value. We also
        // obtain z^2 and z^3 in the process, which we store in a 2-bit
        // window for the end of the exponentiation.
        ...
        if (MQ & 7) == 1 {
            // q = 7 mod 4; square root candidate is computed as:
            //   y <- x^((q+1)/8)
            // We need to process 13 extra exponent bits.
            let e = MQ.wrapping_neg().wrapping_add(1) >> 2;
            for i in 0..6 {
    ...
    pub fn iszero(self) -> u32 {
        ...
        let r = (t0 | t0.wrapping_neg()) & (t1 | t1.wrapping_neg()) & (t2 | t2.wrapping_neg());
    ...
    */
    ...
    pub fn set_decode_reduce(&mut self, buf: &[u8]) {
        *self = Self::ZERO;
        ...
        while n > 0 {
            let k = n - 32;
            let e0 = u64::from_le_bytes(*<&[u8; 8]>::try_from(&buf[k..k + 8]).unwrap());
            let e1 = u64::from_le_bytes(*<&[u8; 8]>::try_from(&buf[k + 8..k + 16]).unwrap());
            let e2 = u64::from_le_bytes(*<&[u8; 8]>::try_from(&buf[k + 16..k + 24]).unwrap());
            let e3 = u64::from_le_bytes(*<&[u8; 8]>::try_from(&buf[k + 24..k + 32]).unwrap());
    ...
    /// Get the "hash" of the value (low 64 bits of the representation).
...
mod tests {
    ...
    fn check_gf_ops<const MQ: u64>(va: &[u8], vb: &[u8], vx: &[u8]) {
        ...
        let zp4 = &zp << 2;
        let zero: BigInt = BigInt::from_bytes_le(Sign::Plus, &[0]);
        ...
        // TODO: this isn't working, seems like
        // zd is negative despite the modular reduction??
        let x = b.0[1] as i32;
        let c = a.mul_small(x);
        let vc = c.encode32();
        let zc = BigInt::from_bytes_le(Sign::Plus, &vc);
        let mut zd: BigInt = (&x * &za) % &zp;
        if zd < zero {
            zd = zd + &zp;
        }
        assert!(zc == zd);
    ...
    fn test_gf<const MQ: u64>(nqr: u32) {
        ...
        assert!(
            GF255::<MQ>::decode_reduce(&va).equals(&GF255::<MQ>::decode_reduce(&vb)) == 0xFFFFFFFF
        );
        ...
        for i in 0..300 {
            ...
            let s = GF255::<MQ>::decode_reduce(&va).square().square();
            if (MQ & 7) == 1 {
                // q = 7 mod 8, we are supposed to get a fourth root
